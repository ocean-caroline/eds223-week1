...
## Why tmap? There are MANY ways to make maps in R, but tmap or “thematic maps” offers the most flexibility.

1.  tmap can handle vector and raster objects from the sf, sp, raster, and stars packages.
2.  The syntax of tmap is based on ggplot2
3.  Grammar of Graphics tmap supports static AND interactive maps (yay!)
...
-   the plotting method (similar to geom_points())

[Let’s load three spatial data objects to plot:]{.underline}
...
#Shapes and layers In tmap, the spatial object to plot needs to be defined within the function tm_shape(). This is analogous to defining the data frame to plot in ggplot2 using ggplot(data = ).
...
![](Screenshot%202024-10-28%20144809.png)
...
![](Screenshot 2024-10-28 145723.png)
